...
const LEADERSHIP_ROLES = {
  "Company": ["Commander", "Executive Officer", "First Sergeant", "Company Admin"],
...
export default function ManageUserUnitDialog({
  ...
  const updateUserMutation = useMutation({
    mutationFn: async (data: { userId: number; unitId: number }) => {
      console.log(`Making API request to reassign user ${data.userId} to unit ${data.unitId}`);
    ...
    onSuccess: (updatedUser) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/hierarchy/accessible-users"] });
  ...
  const fetchUserAssignments = async (userId: number): Promise<UnitAssignment[]> => {
    try {
  ...
  const getLeadershipRolesForUnit = (unitId: number): string[] => {
    ...
    return LEADERSHIP_ROLES[unit.unitLevel as keyof typeof LEADERSHIP_ROLES] || [];
  };
  ...
  const handleMakePrimary = (index: number) => {
    ...
    toast({
      ...
      description: "The primary unit assignment has been updated but not saved yet"
    });
  };
  ...
  const handleRemoveAssignment = (index: number) => {
    ...
    toast({
      ...
      description: "The assignment has been marked for removal but not saved yet"
    });
  };
  ...
  const createAssignmentMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: [`/api/users/${user.id}/unit-assignments`] });
      queryClient.invalidateQueries({ queryKey: ['/api/users'] });
    },
    onError: (error) => {
  ...
  const updateAssignmentMutation = useMutation({
    mutationFn: async ({ assignmentId, updates }: { assignmentId: number, updates: Partial<UnitAssignment> }) => {
      const response = await fetch(`/api/users/${user.id}/unit-assignments/${assignmentId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      for (const assignment of assignmentsToUpdate) {
        ...
        await updateAssignmentMutation.mutate({
          ...
          updates: {
            ...
            leadershipRole: assignment.leadershipRole === "none" ? null : assignment.leadershipRole
          }
        });
      }
        ...
        <Tabs defaultValue="assignments" className="w-full">
          ...
          <TabsContent value="assignments" className="space-y-4 pt-4">
            ...
            <div className="space-y-4">
              {assignments.filter(a => !a.endDate).map((assignment, index) => (
                <Card key={index} className={`${assignment.assignmentType === ASSIGNMENT_TYPES.PRIMARY ? 'border-primary' : ''}`}>
                  <CardHeader className="pb-3">
                    <div className="flex justify-between items-center">
                      <CardTitle className="text-md">{getUnitName(assignment.unitId)}</CardTitle>
                      <Badge variant={assignment.assignmentType === ASSIGNMENT_TYPES.PRIMARY ? "default" : "outline"}>
                  ...
                  <CardContent className="space-y-3 pb-2">
                    <div className="space-y-1">
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="0" disabled>Select a unit</SelectItem>
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value={ASSIGNMENT_TYPES.ATTACHED}>Attached</SelectItem>
                          <SelectItem value={ASSIGNMENT_TYPES.TEMPORARY}>Temporary</SelectItem>
                          <SelectItem value={ASSIGNMENT_TYPES.DUAL_HATTED}>Dual-Hatted</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    ...
                    {newAssignment.unitId > 0 && (
                      <div className="space-y-1">
                        <Label htmlFor="new-role">Leadership Role (Optional)</Label>
                        <Select 
                          value={newAssignment.leadershipRole || "none"}
                          onValueChange={(value) => setNewAssignment({...newAssignment, leadershipRole: value === "none" ? null : value})}
                        >
                          <SelectTrigger id="new-role">
                            <SelectValue placeholder="None (Regular member)" />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value="none">None (Regular member)</SelectItem>
                            {getLeadershipRolesForUnit(newAssignment.unitId).map((role) => (
              ...
              <div className="grid gap-2">
                <div className="flex flex-col gap-2">
                  ...
                  <div className="text-sm space-y-1 text-muted-foreground">
                    ...
                    <p><strong>Primary Unit:</strong> {getUnitName(selectedUnitId)}</p>
