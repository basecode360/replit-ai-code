...
export default function AARDashboard() {
  ...
  const [selectedUnitLevel, setSelectedUnitLevel] = useState<string | null>(
    null
  );

  // Advanced comment filtering options
  const [selectedCommentUnitLevel, setSelectedCommentUnitLevel] = useState<
    string | null
  >(null);
  const [selectedCommentUnitId, setSelectedCommentUnitId] = useState<
    number | null
  >(null);
  ...
  const eventsMap =
    events && Array.isArray(events)
      ? events.reduce((map: Record<number, Event>, event: Event) => {
          map[event.id] = event;
          return map;
        }, {})
      : {};
  ...
  const usersMap =
    users && Array.isArray(users)
      ? users.reduce((map: Record<number, User>, user: User) => {
          map[user.id] = user;
          return map;
        }, {})
      : {};
  ...
  const unitsMap =
    accessibleUnits && Array.isArray(accessibleUnits)
      ? accessibleUnits.reduce((map: Record<number, Unit>, unit: Unit) => {
          map[unit.id] = unit;
          return map;
        }, {})
      : {};
    ...
    aars && Array.isArray(aars)
      ? aars.map((aar: AAR) => {
          return {
            ...
            unitName:
              unitsMap && unitsMap[aar.unitId]
                ? unitsMap[aar.unitId].name
                : "Unknown Unit",
          };
        })
      : [];
  ...
  const groupedAARs = processedAARs.reduce(
    (grouped: Record<number, AAR[]>, aar: AAR) => {
  ...
  const filteredAAREvents = Object.entries(groupedAARs).filter(
    ([eventId, eventAARs]) => {
      ...
      if (
        ...
        (!event.participatingUnits ||
          !event.participatingUnits.includes(selectedUnitId))
      ) {
        return false;
      }
      ...
      if (
        selectedUnitLevel &&
        (!unitsMap[event.unitId] ||
          unitsMap[event.unitId]?.unitLevel !== selectedUnitLevel)
      ) {
        return false;
      }
      ...
      if (searchTerm) {
        ...
        const matchesObjectives =
          event.objectives?.toLowerCase().includes(searchLower) || false;
        const matchesLocation =
          event.location?.toLowerCase().includes(searchLower) || false;

        // Also search through AAR content
        const matchesAARContent =
          Array.isArray(eventAARs) &&
          eventAARs.some((aar) => {
      ...
      if (timeframe !== "all" && event.date) {
        ...
        if (
          ...
          timeframe === "quarter" &&
          now.getTime() - eventDate.getTime() > 90 * 24 * 60 * 60 * 1000
        ) {
          return false;
        }
      }
  ...
  const sortedAAREvents = [...filteredAAREvents].sort(
    ([eventIdA, aarsA], [eventIdB, aarsB]) => {
      ...
      if (sortBy === "recent") {
        return (
          new Date(eventB.date || "").getTime() -
          new Date(eventA.date || "").getTime()
        );
  ...
  const getFilteredItems = (
    eventAARs: AAR[],
    type: "sustainItems" | "improveItems" | "actionItems"
  ) => {
    return eventAARs.flatMap((aar) => {
      ...
      return items.filter((item: AARItem) => {
        ...
        if (
          selectedCommentUnitId !== null &&
          item.unitId !== selectedCommentUnitId
        ) {
          return false;
        }
        ...
        if (
          selectedCommentUnitLevel !== null &&
          item.unitLevel !== selectedCommentUnitLevel
        ) {
          return false;
        }
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <Select
                value={selectedUnitId?.toString() || "_all"}
                onValueChange={(value) =>
                  setSelectedUnitId(value !== "_all" ? Number(value) : null)
                }
              >
                <SelectTrigger>
            ...
            <div>
              <Label htmlFor="unit-level-filter">
                Filter Events by Unit Level
              </Label>
              <Select
                value={selectedUnitLevel || "_all"}
                onValueChange={(value) =>
                  setSelectedUnitLevel(value !== "_all" ? value : null)
                }
              >
                <SelectTrigger>
          ...
          {showAdvancedFilters && (
            <div className="mt-6 pt-6 border-t">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <Label htmlFor="comment-unit-filter">
                    Filter Comments by Unit
                  </Label>
                  <Select
                    value={selectedCommentUnitId?.toString() || "_all"}
                    onValueChange={(value) =>
                      setSelectedCommentUnitId(
                        value !== "_all" ? Number(value) : null
                      )
                    }
                  >
                    <SelectTrigger>
                ...
                <div>
                  <Label htmlFor="comment-unit-level-filter">
                    Filter Comments by Echelon
                  </Label>
                  <Select
                    value={selectedCommentUnitLevel || "_all"}
                    onValueChange={(value) =>
                      setSelectedCommentUnitLevel(
                        value !== "_all" ? value : null
                      )
                    }
                  >
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setSelectedRank(value !== "_all" ? value : null)
                    }
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="Staff Sergeant">
                        Staff Sergeant
                      </SelectItem>
        ...
        {sortedAAREvents.length === 0 ? (
          <Card className="p-8 text-center">
            <p className="text-muted-foreground mb-4">
              No AARs found matching your filters.
            </p>
            <Button
              variant="outline"
              onClick={() => {
                    ...
                    <TabsContent value="all">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 my-4">
                        <Card>
                          ...
                          <CardContent>
                            <ul className="space-y-2">
                              {getFilteredItems(eventAARs, "sustainItems").map(
                                (item: AARItem, i) => (
                                  <li
                                    key={item.id || i}
                                    ...
                                    <div className="flex flex-col">
                                      ...
                                      {typeof item === "object" && (
                                        <span className="text-xs text-muted-foreground mt-1">
                                          {usersMap[item.authorId]?.name ||
                                            "Unknown"}{" "}
                                          ({item.authorRank}) -{" "}
                                          {unitsMap[item.unitId]?.name ||
                                            "Unknown Unit"}
                                        </span>
                                      )}
                                    </div>
                        ...
                        <Card>
                          ...
                          <CardContent>
                            <ul className="space-y-2">
                              {getFilteredItems(eventAARs, "improveItems").map(
                                (item: AARItem, i) => (
                                  <li
                                    key={item.id || i}
                                    className="flex items-start"
                        ...
                        <Card>
                          ...
                          <CardContent>
                            <ul className="space-y-2">
                              {getFilteredItems(eventAARs, "actionItems").map(
                                (item: AARItem, i) => (
                                  <li
                                    key={item.id || i}
                                    className="flex items-start"
                                ...
                                <div>
                                  ...
                                  <div className="text-sm text-muted-foreground mt-1">
                                    {unitsMap[item.unitId]?.name ||
                                      "Unknown Unit"}{" "}
                                    · {item.authorRank}{" "}
                                    {usersMap[item.authorId]?.name || "Unknown"}
                                  </div>
                                </div>
                              </li>
                            )
