...
const demoEvents = [
  {
    ...
    participants: 32,
    objective: "Conduct platoon-level live fire training focused on squad movement techniques and fire coordination."
  },
  {
    id: 2,
    ...
    participants: 8,
    objective: "Practice urban movement and room clearing tactics with emphasis on team coordination."
  },
  {
    id: 3,
    ...
    participants: 128,
    objective: "Execute company-wide maneuvers focusing on combined arms coordination and communication."
  },
  {
    id: 4,
    ...
    participants: 24,
    objective: "Develop leadership skills and tactical decision making for squad and team leaders."
  }
];
      ...
      <div className="flex flex-col space-y-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-muted-foreground">
              Plan, execute, and evaluate your unit's training events in one place
            </p>
          </div>
        </div>
        ...
        <Tabs defaultValue="upcoming" className="space-y-4">
          ...
          <TabsContent value="upcoming" className="space-y-4">
            <div className="event-calendar"> {/* This div is targeted by the tour highlight */}
          ...
          <TabsContent value="active" className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {demoEvents.filter(e => e.step >= 4 && e.step <= 6).map((event) => (
                <EventCard key={event.id} event={event} />
              ))}
            </div>
          </TabsContent>
          ...
          <TabsContent value="completed" className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {demoEvents.filter(e => e.step >= 7).map((event) => (
                <EventCard key={event.id} event={event} />
              ))}
            </div>
          </TabsContent>
    ...
    <Card className="hover:shadow-md transition-shadow cursor-pointer">
      <CardHeader className="pb-2">
        <div className="flex justify-between items-start">
          ...
          <Badge 
            variant={
              event.step >= 7 ? "success" : 
              event.step >= 4 ? "default" : 
              "outline"
            }
          >
            Step {event.step}
...
function TrainingProcess() {
  const steps = [
    ...
    { id: 7, name: "Evaluate", description: "Review and identify lessons learned" },
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          GreenBookAAR tracks each event through the Army's 8-step training model
        </CardDescription>
      </CardHeader>
      <CardContent>
