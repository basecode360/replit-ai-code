...
async function createCompanyLiveFiresEvent() {
  try {
    ...
    console.log(
      `Found ${companySubunits.length} subunits for ${targetCompany.name}`
    );
    ...
    console.log(
      `Company Live Fires event (ID: ${event.id}) has been created with ${companyUsers.length} AARs.`
    );
  } catch (error) {
