...
async function createCompanyLiveFiresEvent() {
  try {
    ...
    // Look for companies
    const companyUnits = allUnits.filter(
      (unit) => unit.unitLevel === "Company"
    );
    ...
    console.log(
      `Found ${companyUnits.length} company-level units: ${companyUnits
        .map((u) => u.name)
        .join(", ")}`
    );
    ...
    console.log(
      `Selected company: ${targetCompany.name} (ID: ${targetCompany.id})`
    );

    // Find all subunits that belong to this company
    const companySubunits = allUnits.filter(
      (unit) => unit.parentId === targetCompany.id
    );
    console.log(
      `Found ${companySubunits.length} direct subunits for ${targetCompany.name}`
    );
    ...
    console.log(
      `Found ${deepSubunits.length} additional subunits (squads/teams) for ${targetCompany.name}`
    );
    ...
    const companyUsers = allUsers.filter((user) =>
      unitIds.includes(user.unitId)
    );

    console.log(
      `Found ${companyUsers.length} users in ${targetCompany.name} and its subunits`
    );
    ...
    const eventCreator =
      companyUsers.find(
        (u) => u.role === "FIRST_SERGEANT" || u.role === "COMMANDER"
      ) || companyUsers[0];
    console.log(
      `Event creator will be: ${eventCreator.name} (${eventCreator.rank}, ${eventCreator.role})`
    );
    ...
    console.log(
      `Company Live Fires event (ID: ${event.id}) has been created with AARs from ${companyUsers.length} users.`
    );
    return event;
  } catch (error) {
...
createCompanyLiveFiresEvent()
  .then((event) => {
    if (event) {
      console.log(
        `Success! Created Company Live Fires event with ID ${event.id}`
      );
    }
    process.exit(0);
  })
