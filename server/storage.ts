...
export interface IStorage {
  ...
  assignUserToUnit(
    assignment: InsertUserUnitAssignment
  ): Promise<UserUnitAssignment>;
  updateUserUnitAssignment(
    id: number,
    updates: Partial<InsertUserUnitAssignment>
  ): Promise<UserUnitAssignment | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async updateUserUnitAssignment(
    id: number,
    updates: Partial<InsertUserUnitAssignment>
  ): Promise<UserUnitAssignment | undefined> {
    try {
  ...
  async getUsersByUnit(unitId: number): Promise<SelectUser[]> {
    try {
      return await db
        .select()
        .from(users)
        .where(and(eq(users.unitId, unitId), eq(users.isDeleted, false)));
    } catch (error) {
  ...
  async getUnitByReferralCode(code: string): Promise<Unit | undefined> {
    try {
      const [unit] = await db
        .select()
        .from(units)
        .where(and(eq(units.referralCode, code), eq(units.isDeleted, false)));
      return unit;
    } catch (error) {
  ...
  async getSubunits(parentId: number): Promise<Unit[]> {
    try {
      return await db
        .select()
        .from(units)
        .where(and(eq(units.parentId, parentId), eq(units.isDeleted, false)));
    } catch (error) {
  ...
  async getEventsByUnit(unitId: number): Promise<Event[]> {
    try {
      ...
      return await db
        .select()
        .from(events)
        .where(and(eq(events.unitId, unitId), eq(events.isDeleted, false)));
    } catch (error) {
  ...
  async getAllEvents(): Promise<Event[]> {
    try {
      ...
      return await db.select().from(events).where(eq(events.isDeleted, false));
    } catch (error) {
    ...
    try {
      ...
      if ("endDate" in processedUpdates) {
        if (
          ...
          processedUpdates.endDate === undefined
        ) {
  ...
  async addUnitToEvent(
    ...
    try {
      ...
      const existingUnits = Array.isArray(event.participatingUnits)
  ...
  async getAAR(id: number): Promise<AAR | undefined> {
    try {
      const [aar] = await db
        .select()
        .from(aars)
        .where(and(eq(aars.id, id), eq(aars.isDeleted, false)));
      return aar;
    } catch (error) {
  ...
  async getAARsByEvent(eventId: number): Promise<AAR[]> {
    try {
      return await db
        .select()
        .from(aars)
        .where(and(eq(aars.eventId, eventId), eq(aars.isDeleted, false)));
    } catch (error) {
  ...
  async getAARsByUnit(unitId: number): Promise<AAR[]> {
    try {
      return await db
        .select()
        .from(aars)
        .where(and(eq(aars.unitId, unitId), eq(aars.isDeleted, false)));
    } catch (error) {
  ...
  async getAARsByUser(userId: number): Promise<AAR[]> {
    try {
      return await db
        .select()
        .from(aars)
        .where(and(eq(aars.createdBy, userId), eq(aars.isDeleted, false)));
    } catch (error) {
  ...
  async updateAAR(
    id: number,
    updates: Partial<InsertAAR>
  ): Promise<AAR | undefined> {
    try {
      const now = new Date();
      const [updatedAAR] = await db
        .update(aars)
        .set({ ...updates, updatedAt: now })
        .where(and(eq(aars.id, id), eq(aars.isDeleted, false)))
        .returning();
  ...
  async generateVenicePromptAnalysis(
    unitId: number,
    prompt: string
  ): Promise<VeniceAnalysis> {
    try {
      ...
      console.log(
        `Analyzing ${unitAARs.length} AARs for unit ${unitId} with prompt: "${prompt}"`
      );
