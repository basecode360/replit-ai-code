...
export class VeniceAIService {
  ...
  public async generateAnalysis(aars: AAR[]): Promise<VeniceAnalysis> {
    ...
    if (aars.length < 3) {
      console.log(`Only ${aars.length} AAR(s) available, suggesting to create more for better analysis`);
      return {
        trends: [
          {
            category: "Insufficient Data",
    ...
    try {
      ...
      console.log(`Analyzing ${aars.length} AARs for trends and patterns using OpenAI`);
  ...
  public async generatePromptAnalysis(aars: AAR[], prompt: string): Promise<VeniceAnalysis> {
    console.log(`Analyzing ${aars.length} AARs for unit with prompt: "${prompt}"`);
  ...
  private parseAnalysisFromText(text: string): VeniceAnalysis {
    ...
    const frictionMatch = text.match(/FRICTION POINTS:([\s\S]*?)(?:RECOMMENDATIONS:|$)/);
   ...
   */
  private getPromptSpecificDefaultAnalysis(prompt: string, errorMessage?: string): VeniceAnalysis {
    return {
      trends: [
        {
   ...
   */
  public async generateCustomAnalysis(aars: AAR[], events: Event[], userPrompt: string): Promise<{ content: string }> {
    ...
    try {
      ...
      const formattedEvents = events.map(event => {
        return {
          ...
          date: typeof event.date === 'string' ? event.date.split('T')[0] : new Date(event.date as any).toISOString().split('T')[0],
          step: event.step || 0,
      ...
      console.log(`Generating custom analysis using OpenAI (${aars.length} AARs, ${events.length} events)`);
